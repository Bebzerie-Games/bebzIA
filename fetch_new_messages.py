...
# ... (importations et définitions de variables globales en haut du fichier) ...
# ... (définition de send_log_message, main_fetch_logic, format_message_to_json) ...

async def run_script():
    global LOG_CHANNEL_ID # Ensure it's accessible
    try:
        print("[DEBUG] run_script: Début de la fonction run_script.") # <<<<<<< AJOUT
        print(f"[DEBUG] run_script: Vérification du DISCORD_BOT_TOKEN: {'Présent' if DISCORD_BOT_TOKEN else 'MANQUANT'}") # <<<<<<< AJOUT
        print(f"[DEBUG] run_script: Vérification du LOG_CHANNEL_ID_STR: {LOG_CHANNEL_ID_STR}") # <<<<<<< AJOUT

        print("[DEBUG] run_script: Tentative de discord_log_client.login()...") # <<<<<<< AJOUT
        await discord_log_client.login(DISCORD_BOT_TOKEN)
        print("[DEBUG] run_script: discord_log_client.login() terminé.") # <<<<<<< AJOUT

        print("[DEBUG] run_script: Tentative de discord_log_client.wait_until_ready()...") # <<<<<<< AJOUT
        await discord_log_client.wait_until_ready()
        # Cette ligne ne s'affichera que si wait_until_ready se termine
        print(f"[DEBUG] run_script: discord_log_client.wait_until_ready() terminé. Client Discord pour les logs connecté en tant que {discord_log_client.user}") # <<<<<<< AJOUT
        ...
        if discord_log_client.is_ready():
            print("[DEBUG] run_script: Tentative de déconnexion du discord_log_client...") # <<<<<<< AJOUT
            await discord_log_client.close()
            print("[DEBUG] run_script: discord_log_client déconnecté.") # <<<<<<< AJOUT
        else:
            print("[DEBUG] run_script: discord_log_client n'était pas prêt, pas de déconnexion nécessaire ou possible proprement.") # <<<<<<< AJOUT
        print("Script de récupération des messages terminé (run_script).")


if __name__ == "__main__":
    print("Démarrage de run_script...") # Ce print est celui que vous voyez
    try:
        asyncio.run(run_script()) # C'est ici que l'exécution entre dans la fonction ci-dessus
    except KeyboardInterrupt:
        print("Script interrompu manuellement (KeyboardInterrupt).")
    except Exception as e:
        print(f"Erreur non gérée au niveau de asyncio.run : {e}")
